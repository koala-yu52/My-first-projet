    ...
    , ui(new Ui::MainWindow)
{
    ...
    // 顺便确保它不会因为太小而不显示字体
    ui->label_alarm->setMinimumHeight(60);
    // --- 警报定时器升级版 ---
   
    flashTimer = new QTimer(this);
    connect(flashTimer, &QTimer::timeout, [=]() {
        // 1. 强制让 Label 每一秒都确认自己铺满了整个登录页面
        ui->label_alarm->setGeometry(0, 0, ui->page_login->width(), ui->page_login->height());

        if (flashStep % 2 == 0) {
            // 步骤为偶：纯黑背景 + 浅红色字
            ui->label_alarm->setStyleSheet(
                "background-color: #000000; " // 纯黑背景，盖掉一切
                "background-image: none; "    // 强制去掉校徽水印
                "color: #FF8080; "           // 浅红色
                "font-size: 100px; "         // 巨大字体
                "font-weight: 900; "
        ...
        if (flashStep > 100) { // 30秒
            flashTimer->stop();
            ui->label_alarm->hide();
            ui->loginPanel->show(); // 恢复零件
            // 恢复页面原本的样式（如果有的话）
            ui->page_login->update();
        }
        });
...
// 让表格列宽自动填满
    ...
    // 2. 翻页逻辑
    // --- 统一返回逻辑：所有返回按钮都指向第 1 页 (主页) ---
    // 2. 【核心跳转逻辑】从主页进入各个房间
    
    connect(ui->btn_go_mean, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(2); });
    connect(ui->btn_go_stats, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(3); });
    connect(ui->btn_go_A, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(4); });
    connect(ui->btn_go_B, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(5); });
    connect(ui->btn_go_ind, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(6); });
    connect(ui->btn_go_fit, &QPushButton::clicked, [=]() { ui->mainStackedWidget->setCurrentIndex(7); });
...
void MainWindow::on_btn_calc_mean_clicked() {
    ...
    // --- 开始构造精美 HTML 报告 ---
    QString html = "<h2 style='color:#741B47; text-align:center;'>平均值计算报告</h2><hr>";
    ...
    html += "<div style='background-color:#e8f8f5; padding:10px; border:1px solid # #741B47; border-radius:5px; margin-top:5px;'>"
...
void MainWindow::on_btn_calc_stats_clicked() {
    ...
    QString html = "<h2 style='color: #741B47; text-align:center;'>统计分析报告</h2><hr>";
    ...
    resBox += "<span style='font-size:18pt; color:# #741B47; font-weight:bold;'>标准差 <i>S</i> &asymp; ";
...
void MainWindow::on_btn_calc_A_clicked() {
    ...
    // --- 构造精美 HTML 报告 ---
    QString html = "<h2 style='color: #741B47; text-align:center;'>A类不确定度分析报告</h2><hr>";
...
void MainWindow::on_btn_calc_B_clicked() {
    ...
    // --- 构造精美 HTML 报告 ---
    QString html = "<h2 style='color: #741B47; text-align:center;'>B类不确定度分析报告</h2><hr>";
    ...
    html += "<br><b style='font-size:11pt;'>【最终结果】：</b><br>";
    QString resBox = "<div style='background-color: #741B47; padding:12px; border:2px solid #3498db; border-radius:5px;'>";
...
void MainWindow::on_btn_calc_ind_clicked() {
    ...
    try {
        ...
        // 3. 构建报告内容
        QString html = "<h2 style='color:# #741B47; text-align:center;'>间接不确定度分析报告</h2><hr>";
        ...
        resBox += "<span style='font-size:15pt; color: #741B47;'>间接不确定度 <i>u<sub>y</sub></i> &asymp; <b>";
...
void MainWindow::on_btn_run_fit_clicked() {
    ...
    // 展示精美 HTML 报告
    ui->text_res_fit->setHtml("<h3 style='color: #741B47;'>曲线拟合分析报告</h3>"
...
void MainWindow::on_btn_verify_clicked() {
    QString userAnswer = ui->edit_answer->text().trimmed();
    const QString correctAnswer = "允公允能，日新月异";

    if (userAnswer == correctAnswer) {
        if (flashTimer) flashTimer->stop();
        ui->label_alarm->hide();
        if (ui->loginPanel) ui->loginPanel->show();
        ui->mainStackedWidget->setCurrentIndex(1);
        ui->edit_answer->clear();
    }
    else {
        // --- 开启警报 ---
        if (ui->loginPanel) ui->loginPanel->hide();

        ui->label_alarm->setText("⚠️警告：非法入侵！！");

        // 关键：强制铺满并置于最顶层
        ui->label_alarm->setGeometry(ui->page_login->rect());
        ui->label_alarm->show();
        ui->label_alarm->raise(); // 这一行能确保它压在校徽水印上面

        flashStep = 0;
        if (flashTimer) flashTimer->start(300);
        ui->edit_answer->clear();
    }
}